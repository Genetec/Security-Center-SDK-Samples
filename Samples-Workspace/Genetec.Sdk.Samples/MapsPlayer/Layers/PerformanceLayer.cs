...
namespace MapsPlayer.Layers
{
    internal class PerformanceLayer : MapLayer
    {

        #region Public Fields

        public static readonly Guid Identifier = new Guid("B854C2F0-47BB-49D0-8C63-BE085DADBB0D");
        ...
        /// </summary>
        public override string Name => "AEOS Doors";
        ...
        public override void Clear()
        {
            Action pFunc = delegate
            {
                lock (m_syncRoot)
                {
                    ...
                    foreach (var mapObjectVisual in m_layer.Visuals)
                    {
                        IDisposable disposableMapObjectVisual = mapObjectVisual as IDisposable;

                        disposableMapObjectVisual?.Dispose();
                    }
        ...
        public override void Reposition(IEnumerable<IMapObjectView> items)
        {
            ...
            Action pFunc = delegate
            {
                ...
                {
                    if (item is DoorVisualView visual)
                    {
        ...
        {
            var size = Size;
            if (!double.IsInfinity(size.Height) && !double.IsNaN(size.Height) && !double.IsInfinity(size.Width) && !double.IsNaN(size.Width))
            {
                Action pFunc = delegate
                {
        ...
        private void Init()
        {
            ...
            new DispatcherTimer(TimeSpan.FromSeconds(1), DispatcherPriority.Normal, OnTimerDigestTick, s_dispatcher);
            s_counter++;
        }
