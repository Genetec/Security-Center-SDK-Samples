...
namespace DronesTracker.Maps.Layers
{
    public sealed class MotionHeatMapLayer : MapLayer
    {
        ...
        /// </summary>
        public override Guid Id => new Guid("{EF40F45C-1CD5-43C7-9FF9-5CB473C66F5A}");
        ...
        /// </summary>
        public override string Name => "Motion Heat Map";
        ...
        public MotionHeatMapLayer(Genetec.Sdk.Workspace.Workspace workspace, DemoMapObjectProvider provider)
        {
            m_workspace = workspace;
            provider.NewPosition += OnProviderNewPosition;
        }
        ...
        {
            return m_panel ?? (m_panel = new HeatMapPanel(m_workspace, this));
        }
        ...
        {
            Dispatcher?.BeginInvoke(new Action(m_panel.InvalidateVisual));
        }
                        ...
                        if (scale > 0.0025)
                        {
                            lock (m_layer.SyncRoot)
                            {
                                foreach (var kv in m_layer.Positions)
                                {
                                    ...
                                    var pixel = m_layer.CoordinateToPixel(coords);
