            ...
            try
            {
                foreach (var param in postParameters)
                {
                    ...
                    else if (param.Value is PayloadParameter payload)
                    {
                        ...
                        // The IFormFile FileName SHALL be specified in the multipart Content-Disposition otherwise a BadRequest HTTP 400 is returned
                        // - https://github.com/dotnet/aspnetcore/issues/19295
                        ...
                        // Add just the first part of this param, since we will write the file data directly to the Stream
                        var dummyFilename = "dummyFilename.txt";
                        var header = $"--{new StringBuilder(boundary).Append(Environment.NewLine)}Content-Disposition: form-data; name=\"{param.Key}\"; filename=\"{dummyFilename}{param.Key}\"{Environment.NewLine}Content-Type: {new StringBuilder(payloadToUpload.ContentType ?? "application/octet-stream").Append(Environment.NewLine).Append(Environment.NewLine)}";
